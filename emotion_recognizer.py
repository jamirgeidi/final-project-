...
def get_emotion_from_cloud(image_path):
    ...
    elif str(response.face_annotations[0].joy_likelihood) == "Likelihood.LIKELY": 

        app_tables.data.add_row(Mood= "Joy", Date=datetime.now())

    elif str(response.face_annotations[0].joy_likelihood) == "Likelihood.POSSIBLE": 

        app_tables.data.add_row(Mood= "Joy", Date=datetime.now())

    elif str(response.face_annotations[0].sorrow_likelihood) == "Likelihood.VERY_LIKELY": 

        app_tables.data.add_row(Mood= "Sorrow", Date=datetime.now())

    elif str(response.face_annotations[0].sorrow_likelihood) == "Likelihood.LIKELY": 

        app_tables.data.add_row(Mood= "Sorrow", Date=datetime.now())

    elif str(response.face_annotations[0].sorrow_likelihood) == "Likelihood.POSSIBLE": 

        app_tables.data.add_row(Mood= "Sorrow", Date=datetime.now())    

    elif str(response.face_annotations[0].anger_likelihood) == "Likelihood.VERY_LIKELY": 

        app_tables.data.add_row(Mood= "Anger", Date=datetime.now())

    elif str(response.face_annotations[0].anger_likelihood) == "Likelihood.LIKELY": 

        app_tables.data.add_row(Mood= "Anger", Date=datetime.now())
   
    elif str(response.face_annotations[0].anger_likelihood) == "Likelihood.POSSIBLE": 

        app_tables.data.add_row(Mood= "Anger", Date=datetime.now())

    elif str(response.face_annotations[0].surprise_likelihood) == "Likelihood.VERY_LIKELY": 

        app_tables.data.add_row(Mood= "Shocked", Date=datetime.now())

    elif str(response.face_annotations[0].surprise_likelihood) == "Likelihood.LIKELY": 

        app_tables.data.add_row(Mood= "Shocked", Date=datetime.now())

    elif str(response.face_annotations[0].surprise_likelihood) == "Likelihood.POSSIBLE": 

        app_tables.data.add_row(Mood= "Shocked", Date=datetime.now())

    else:
        print("No emotion detcted, try again") 
